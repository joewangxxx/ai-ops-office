...
describe('static office map', () => {
  it('renders the complete initial office state from the layout data', () => {
    ...
    expect(within(hub).getByText('PRD 2')).toBeInTheDocument();
    expect(within(hub).getByText('Feature 3')).toBeInTheDocument();
    expect(within(hub).getByText('Report 1')).toBeInTheDocument();

    const renderedSources = Array.from(document.querySelectorAll('img')).map((image) => image.getAttribute('src'));
    expect(renderedSources).toContain(`/${officeLayout.assetAnchors.furniture.deskStandard.path.replace(/^images\//, '')}`);
