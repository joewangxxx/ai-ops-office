...
export function storyScenarioForState(input: OfficeStoryState | StoryRuntime): DemoScenario {
  ...
  const devInProgressArtifactIds = jackIsCoding
    ? ['login-feature-v1', 'account-security-feature-v1', 'audit-export-feature-v1']
    : ['account-security-feature-v1', 'audit-export-feature-v1', 'notification-center-feature-v1'];
  ...
  return {
    ...
    workspaces: demoScenario.workspaces.map((workspace) => {
      if (workspace.workspaceId === 'pm-office') return {
          ...workspace,
      ...
      if (workspace.workspaceId === 'dev-office') return {
        ...workspace,
        todayOutput: [{ label: 'Features In Progress', artifactIds: devInProgressArtifactIds }],
      };
      return workspace;
    }),
